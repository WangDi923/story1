...
def _dispatch_intent(nlu: NLUResult, raw: str, state: GameState,
    ...
    target = normalize_entity(entities.get("target", ""))
    location = normalize_entity(entities.get("location", ""))

    # 新版NLU常把“松果”放在target、“草丛”等地点放在location。
    # 为兼容旧剧情推进，EXPLORE/GIVE在必要时改用location作为动作目标。
    if not target and location:
        target = location
    elif intent == "EXPLORE" and target == "pine_nuts" and location:
        target = location
    elif intent == "GIVE" and target == "pine_nuts" and location in ALL_NPCS:
        target = location
